...
| Only one table of the pair exists with a matching owner record | Accept the attributable mapping, then let the approved DB-06 lifecycle retry only the missing table once before DML. | Creating the missing half inside naming detection or deleting the surviving table as compensation. |
| Only one table of the pair exists without a valid matching owner record | Reject it as ambiguous and require explicit operator resolution. | Guessing ownership or implicitly creating, adopting, or deleting either table. |
